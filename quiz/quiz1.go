...
// CheckQuizzes quiz1 answer
func CheckQuizzes() {
	csvFileName := flag.String("csv", "problems.csv", "It's a csv file with 'Questsions and Answers'")
	timerLimit := flag.Int("limit", 10, "Its a timer limit for Quiz")
	flag.Parse()
	file, err := os.Open(*csvFileName)
	if err != nil {
		Exit(fmt.Sprintf("Failed to open the file: [%s]", err))
	}
